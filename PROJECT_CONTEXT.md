...
- The experimental browser Player Profile V2 was retired on 2026-06-15. The existing player profile popup is the only supported browser player-detail UI; do not restore the old A/B toggle or V2 feature flags without a new explicit request.
...
- Utility templates:
  - `gen_alpha_fee_direct_pay_v2`: direct Pay Now utility template for fee reminders. It opens `pay.html?e={{1}}` and keeps plan selection on the payment page; body wording does not mention 1/3/6 month choice (that happens on the payment page). Supersedes `gen_alpha_fee_direct_pay`. Use the older plan-button templates as fallback while this template is missing, disabled, or awaiting Meta approval.
...
- Do not expose wrong-number controls in normal player UI. Only flagged players get a correction note beside the phone field; saving a different valid 10-digit number reactivates future reminders.
